	...
	<xsl:template match="NewDataSet">
		<xsl:for-each select="RediscoveryExport">
			...
			<xsl:for-each select="document('series.xml')/NewDataSet/RediscoveryExport[Collection_Nbr = $coll]">
				...
				<xsl:if test="document('fileunit.xml')/NewDataSet/RediscoveryExport[Collection_Nbr = $coll][Series_Nbr = $series][matches(File_Unit_Nbr,'^\d{4}$')]">
					...
					<xsl:result-document href="children/box/{$path}.json" method="text">
						...
						<xsl:for-each select="document('fileunit.xml')/NewDataSet/RediscoveryExport[Collection_Nbr = $coll][Series_Nbr = $series][matches(File_Unit_Nbr,'^\d{4}$')]">
							<xsl:variable name="component_id" select="concat($coll,'.',$series,'.',File_Unit_Nbr)"/>
			...
			<xsl:if test="document('fileunit.xml')/NewDataSet/RediscoveryExport[Collection_Nbr = $coll][not(Series_Nbr)][matches(File_Unit_Nbr,'^\d{4}$')]">
				...
				<xsl:result-document href="children/box/{$path}.json" method="text">
					...
					<xsl:for-each select="document('fileunit.xml')/NewDataSet/RediscoveryExport[Collection_Nbr = $coll][not(Series_Nbr)][matches(File_Unit_Nbr,'^\d{4}$')]">
						<xsl:variable name="component_id" select="concat($coll,'.',File_Unit_Nbr)"/>
						<xsl:call-template name="child">
							<xsl:with-param name="parent" select="$parent"/>
							<xsl:with-param name="uri" select="$uri"/>
							<xsl:with-param name="component_id" select="$component_id"/>
