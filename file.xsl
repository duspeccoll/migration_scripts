	...
	<xsl:template match="NewDataSet">
		<xsl:for-each select="RediscoveryExport">
			<xsl:variable name="component_id">
				<xsl:choose>
					<xsl:when test="Series_Nbr">
						<xsl:value-of select="concat(Collection_Nbr,'.',Series_Nbr,'.',File_Unit_Nbr)"/>
					</xsl:when>
					<xsl:otherwise>
						<xsl:value-of select="concat(Collection_Nbr,'.',File_Unit_Nbr)"/>
					</xsl:otherwise>
				</xsl:choose>
			</xsl:variable>
			...
			<xsl:choose>
				<xsl:when test="not(Title)">
					<xsl:choose>
						<xsl:when test="contains(File_Unit_Nbr,'\.')">
							<xsl:variable name="x" select="replace(substrinf-after(File_Unit_Nbr,'.'),'^0+','')"/>
